...
@patch("prometheus_client.requests.get")
def test_prometheus_client_query_range_http_error(mock_get: Mock) -> None:
    """query_range() should raise exception on HTTP error."""
    config = _make_prometheus_config()
    client = PrometheusClient(config)
...
@patch("prometheus_client.requests.get")
def test_prometheus_client_query_range_http_error_text_read_fails(
    mock_get: Mock,
) -> None:
    """query_range() should handle HTTP error even when reading response.text fails."""
    config = _make_prometheus_config()
    client = PrometheusClient(config)
    ...
    mock_response.status_code = 500
    mock_response.url = "http://prom:9090/api/v1/query_range"
    # Simulate error when reading response.text by making it raise an exception
    ...
    # Should still raise HTTPError even if reading text fails
    with pytest.raises(requests.HTTPError, match="Server error"):
        client.query_range("up", start=1700000000, end=1700000300, step="300s")


@patch("prometheus_client.requests.get")
def test_prometheus_client_query_range_invalid_json(mock_get: Mock) -> None:
    """query_range() should raise exception on invalid JSON."""
    config = _make_prometheus_config()
    client = PrometheusClient(config)
...
@patch("prometheus_client.requests.get")
def test_prometheus_client_query_range_non_dict_response(mock_get: Mock) -> None:
    """query_range() should raise exception when response is not a dict."""
    config = _make_prometheus_config()
    client = PrometheusClient(config)
...
@patch("prometheus_client.requests.get")
def test_prometheus_client_query_range_without_extra_log_fields(mock_get: Mock) -> None:
    """query_range() should work when extra_log_fields is not provided (None)."""
    config = _make_prometheus_config()
    client = PrometheusClient(config)
